        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-4 text-right">
              <a class="btn btn-info btn-sm mt-1" href="{{ route('backend.inside-details.create') }}">
                <i class="bx bx-plus font-size-16 align-middle mr-1"></i>เพิ่ม Inside Details
              </a>
            </div>
          </div>
  ...
  <script>
    ...
    $(function () {
      oTable = $('#data-table').DataTable({
        ...
        iDisplayLength: 25,
        ajax: {
          url: '{{ action("Backend\Safety\BasicApproach\InsideDetailController@Datatable") }}',
          data: function (d) {
            d.Where={};
        ...
        rowCallback: function (nRow, aData, dataIndex) {
          $('td:last-child', nRow).html(''
            + '<a href="{{ route('backend.inside-details.index') }}/' + aData['id'] + '/edit" class="btn btn-sm btn-primary"><i class="bx bx-edit font-size-16 align-middle"></i></a> '
            {{--+ '<a href="javascript: void(0);" data-url="{{ route('backend.inside-details.index') }}/' + aData['id'] + '" class="btn btn-sm btn-danger cDelete"><i class="bx bx-trash font-size-16 align-middle"></i></a>'--}}
          ).addClass('input');
          $('td:nth-child(2)', nRow).html(''
          + '<div class="container">\n' +
            '  <img src="{{"local/public/images/Safety/BasicApproach/InsideDetails/Image/"}}'+aData['image']+'" class="rounded" alt="Cinque Terre" width="auto" height="200" > \n' +
            '</div>'
