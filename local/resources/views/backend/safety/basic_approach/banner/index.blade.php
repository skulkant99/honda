        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-4 text-right">
              <a class="btn btn-info btn-sm mt-1" href="{{ route('backend.banner.create') }}">
  ...
  <script>
    ...
    $(function() {
      oTable = $('#data-table').DataTable({
        ...
        ajax: {
          url: '{{ action("Backend\Safety\BasicApproach\BannerController@Datatable") }}',
          data: function ( d ) {
            d.Where={};
        ...
        rowCallback: function(nRow, aData, dataIndex){
          $('td:last-child', nRow).html(''
            + '<a href="{{ route('backend.banner.index') }}/'+aData['id']+'/edit" class="btn btn-sm btn-primary"><i class="bx bx-edit font-size-16 align-middle"></i></a> '
            {{--+ '<a href="javascript: void(0);" data-url="{{ route('backend.banner.index') }}/'+aData['id']+'" class="btn btn-sm btn-danger cDelete"><i class="bx bx-trash font-size-16 align-middle"></i></a>'--}}
          ).addClass('input');
          // console.log("nRow : ", nRow)
          $('td:nth-child(2)', nRow).html(''
            + '<div class="container">\n' +
            '  <img src="{{"local/public/images/Safety/BasicApproach/Banner/"}}'+aData['image']+'" class="rounded" alt="Cinque Terre" width="auto" height="200" > \n' +
            '</div>'
          );
        }
      });
